...
контакт → **телефон (обязательно)** → сводка → ✅ Отправить / ✏️ Исправить / ❌ Отмена.
Если включён один отдел — шаг выбора исполнителя пропускается.
...
- `notification_chat_id` — **обязательно** (иначе уведомления не пойдут). Можно вписать сюда, либо
  оставить пустым и задать через .env — см. ниже. **Несколько получателей** — списком
  (`[111, 222]`) или через запятую (`"111, 222"`); уведомление и файлы уйдут во все чаты.
...
- `GET /api/registry` → включённые отделы и их типы обращений (минимальный справочник для формы).
- `GET /api/catalog` → полный справочник: отделы+типы, подразделения-заявители, приоритеты
  и актуальные лимиты файлов/описания. Клиенты не должны хардкодить эти справочники.
- `GET /api/tickets/{id}` → одна заявка и её кешированный статус. Если передан заголовок
  `X-KNUS-Client`, возвращаются только заявки с `source=external:<этот client>`.
